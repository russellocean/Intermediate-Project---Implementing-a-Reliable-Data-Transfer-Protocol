...
class NetworkSimulator:
    ...
    def Simulate(self):
        ...
        while self.continue_simulation:
            # print("Simulation loop - Remaining Events: ", len(self.event_list))
            ...
            if len(self.event_list) == 0:
                ...
                if cur_event.evtype == EventType.FROM_APPLICATION_LAYER:
                    ...
                    self.print_entity_message(
                        cur_event.eventity,
                        "Rcvd from Application Layer: %s" % payload,
                        None,
                    )
                    self.print_to_log(
                        cur_event.eventity,
                    ...
                    self.Host[cur_event.eventity].receive_from_application_layer(
                        payload
                    )
                ...
                # This is an event being passed up from the network layer
                    ...
                    # Log this event
                    self.print_entity_message(
                        cur_event.eventity, "Rcvd from Network Layer", cur_event.pkt
                    )

                    if not self.packet_is_ack(cur_event.pkt):
                        self.print_to_log(
                            self.opposite_entity(cur_event.eventity),
                            cur_event.eventity,
                            "Rcvd from Network Layer",
                            cur_event.pkt,
                        )
                    else:
                        self.print_to_log(
                    ...
                    # Send this message to the assigned host
                    self.Host[cur_event.eventity].receive_from_network_layer(
                        cur_event.pkt
                    )
                ...
                # This is a timer interrupt event
                    ...
                    self.print_to_log(
                        cur_event.eventity, cur_event.eventity, "Timer Interrupt", None
                    )
    ...
    def generate_next_arrival(self):
        if self.num_events < self.max_events:
            ...
            x = (
                self.arrival_rate * random.uniform(0.0, 1.0) * 2
    ...
    def pass_to_network_layer(self, entity, packet):
        ...
        if is_ACK:
            self.print_to_log(
                self.opposite_entity(entity), entity, "Passing to Network Layer", packet
            )
        else:
        ...
        if random.uniform(0.0, 1.0) < self.lossprob:
            ...
            if is_ACK:
                self.print_to_log(
                    self.opposite_entity(entity), entity, "LOSING PACKET!", packet
                )
            else:
        ...
        new_event.eventity = EventEntity(
            (int(entity) + 1) % 2
        )  # event occurs at the other entity
        new_event.pkt = pkt
        ...
        if random.uniform(0.0, 1.0) < self.corruptprob:
            ...
            if is_ACK:
                self.print_to_log(
                    self.opposite_entity(entity), entity, "CORRUPTING PACKET!", packet
                )
            else:
    ...
    def pass_to_application_layer(self, entity, data):
        ...
        self.print_entity_message(
            entity, "Passing to Application Layer: %s" % data, None
        )
        self.print_to_log(
            self.opposite_entity(entity),
            entity,
            "Passing to Application Layer: %s" % data,
            None,
        )
    ...
    def start_timer(self, entity, increment):
        ...
        for e in self.event_list:
            if e.evtype == EventType.TIMER_INTERRUPT and e.eventity == entity:
                self.print_entity_message(
                    entity,
                    "WARNING: ATTEMPTED TO START TIMER WHILE ONE IS ALREADY RUNNING",
                    None,
                )
                self.print_to_log(
                    entity,
        ...
        for idx, e in enumerate(self.event_list):
            if e.eventity == entity:
                if e.evtype == EventType.TIMER_INTERRUPT:
                    ...
                    pass
        self.print_entity_message(
            entity, "ERROR: ATTEMPTED TO STOP A TIMER BUT NONE WERE RUNNING", None
        )
        self.print_to_log(
            entity,
            entity,
            "WARNING: ATTEMPTED TO STOP A TIMER BUT NONE WERE RUNNING",
            None,
        )
